...
The package ships with the `VixPHPCS` ruleset plus additional standalone sniffs you can enable manually. The default ruleset includes object-oriented checks such as requiring `Stringable` on classes that declare `__toString()`. See [docs/SNIFFS.md](docs/SNIFFS.md) for the full catalog, examples, and configurable parameters.
## Development
