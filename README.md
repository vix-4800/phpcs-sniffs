...
The package ships with the `VixPHPCS` ruleset plus additional standalone sniffs you can enable manually. The default ruleset includes operator consistency rules such as preferring `&&` and `||` over `and` and `or`. See [docs/SNIFFS.md](docs/SNIFFS.md) for the full catalog, examples, and configurable parameters.
